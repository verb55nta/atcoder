...
int main(void){
  ...
  vector<ll> c(N);
  //vector<ll> dp(N);
  vector<ll> ele_min(N,INF);
  //map<ll,ll> ele_min;
  rep(i,N) ele_min[i]=INF;
  ...
  rep(i,N){
    ...
    if(i==0){
      ...
      *lower_bound( begin(ele_min),end(ele_min) , c[i] )=c[i];

    }
    
  }

  cout << N- distance( begin(ele_min),lower_bound( begin(ele_min) , end(ele_min) , INF ) ) << endl;
